...
compare_target_to_focal <- function(con, quesp, tarsp, tarname, gene_tag){
  ...
  synmap <- synder::read_synmap(con@input@syn[[tarname]], 
                                seqinfo_a=qseqinfo, 
                                seqinfo_b=tseqinfo)
...
secondary_data <- function(con, quesp, tarsp, quename, tarname){
  gene_tag <- load_gene_list(con@input@query_gene_list)
  gene_check(quesp$mRNA, quename, gene_tag)
  pair <- compare_target_to_focal(con, quesp, tarsp, tarname, gene_tag)
  pair <- calculate_match_significance(pair,con)
  saveRDS(pair,paste0(con@archive,"/",quesp,"-",tarsp,".rds"))
  pair
}
