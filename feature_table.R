...
buildFeatureTable <- function(con, pair){
  ...
  feature_table <- data.frame(
    ...
    gen = genes %in% subset(aa2aa, pval.adj < p2p_cutoff)$query,
    # ORF match in SI
    orf = genes %in% subset(aa2orf, pval.adj < p2a_cutoff)$query,
    # ORF match to spliced transcript (possibly multi-exonic)
    trn = genes %in% subset(aa2transorf, pval.adj < p2t_cutoff)$query,
